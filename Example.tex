...
%
\begin{figure}\centering\subfloat[$\mathcal Y_\infty^{[0,\alpha^\ast]}$]{%
\begin{tikzpicture}
...
\draw[dashed] (2,-.2) node[below] {$2$} -- (2,1.2);
\draw[dotted] (2,1) -- (0,0) -- (-2,1);
\draw (-1.55,.8) -- (-.775,.4);
\draw (1.55,.8) -- (.775,.4);
\fill[pattern = north east lines, opacity=0.8] (.8525,.4) -- (1.705,.8) -- (-1.705,.8) -- (-.8525,.4) -- cycle;
\draw (.8525,.4) -- (1.705,.8) -- (-1.705,.8) -- (-.8525,.4) -- cycle;
\draw[latex'-] (.5,.6) to[out=60,in=180] (1,1.5) node[right] {$(1+\epsilon)\alpha\mathcal R_4^1$};
\draw[latex'-] (.4,.2) to[out=-30,in=90] (.7,-.1) node[below] {$\alpha\mathcal R_\infty^1$};
\end{tikzpicture}%
}~~
\subfloat[$\mathcal Z_\infty^{[0,\alpha^\ast]}$]{%
\begin{tikzpicture}
	\draw[-latex'] (0,0) node[below] {$0$} -- (0,2) node[right] {$\alpha$};
	\draw[-latex'] (-2.2,0) -- (2.2,0) node[right] {$x$}; 
...
\caption{ for $x(k+1) = \frac{1}{2}x(k) + w$.}\label{fig:first:example}
\end{figure}
%
