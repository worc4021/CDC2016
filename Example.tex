...
%
Although we can explicitly calculate $\mathcal Y_\infty^{[0,1]}$ we illustrate the approximation using a finite iterate~$\mathcal Y_M^{[\underline\alpha,\bar\alpha]}$ in Figure~\ref{fig:first:example}, for comparison the set~$\mathcal Z_\infty^{[0,1]}$ is also depicted.
...
\draw (-1.55,.8) -- (-.3875,.2);
\draw (1.55,.8) -- (.3875,.2);
\fill[pattern = north east lines, opacity=0.8] (.4263,.2) -- (1.705,.8) -- (-1.705,.8) -- (-.4263,.2) -- cycle;
...
\subfloat[$\mathcal Z_1^{[0,1]}=\mathcal Z_\infty^{[0,1]}$]{%
\begin{tikzpicture}[scale=1.1]
	\draw[-latex'] (0,0) node[below] {$0$} -- (0,2) node[right] {$\alpha$};
	\draw[-latex'] (-2.2,0) -- (2.2,0) node[right] {$x$};
	\fill[pattern = north east lines, opacity=0.8] (-2,0) -- (2,0) -- (2,1) -- (-2,1) -- cycle;
	\draw[thick] (-2,0) -- (2,0) -- (2,1) -- (-2,1) -- cycle;
	\draw (2,0) -- (2,1);
	\draw (.1,1) -- (-.1,1) node[above right] {$1$};
	\draw[dashed] (-2,-.1) node[below] {$-2$} -- (-2,1.2);
	\draw[dashed] (2,-.1) node[below] {$2$} -- (2,1.2);
...
\caption{Both RPI sets for example system A can be calculated explicitly for all $\alpha\leq\alpha^\ast$. We illustrate the approximation to $\mathcal Y_\infty^{[0.2,0.8]}$. The MRPI set $\mathcal Z^{[0,1]}_\infty$ is determined after one iteration and is given by the state constraints.}
