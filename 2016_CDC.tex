...
Now, in this study, we analyze how RPI sets for system~\eqref{eq:system} with constraints~\eqref{eq:constraints} change with the scaling $\alpha$. More precisely, we intend to simultaneously describe a family of RPI sets for a given interval $[\lb{\alpha},\ub{\alpha}]$ of scaling factors. 
...
We next consider system~\eqref{eq:system} with 
$$A=\begin{pmatrix}
\blind{+}0.6696 & \blind{+}0.3369  \\ -0.6609 & -0.3261
\end{pmatrix}$$
and the constraints 
$$
\X = \{x \in \R^2 \,|\,  -1 \leq K x \leq 1, \, -4 \leq x_2 \leq 2 \}
$$
and $\W^\ast=\{ w \in \R^2 \,|\, \|w\|_\infty \leq 0.1 \}$ with 
$$
K:=\begin{pmatrix}
-0.6609 &  -1.3261
\end{pmatrix}.
$$
The example results from the system
$$
	x(k+1) = \begin{pmatrix} 1 &1 \\ 
	0& 1
	\end{pmatrix} x(k) + \begin{pmatrix} 0.5 \\1.0 \end{pmatrix} u(k) + w(k)
$$
with the controller $u(k)=K\,x(k)$ and the input constraints $\|u(k)\| \leq 1$ as discussed in \cite[Sect.~4.1]{Mayne2005} (without the artificial state constraint $-4 \leq x_2$ that makes $\X$ a C-set).

We now follow the same steps as in Sect.~\ref{subsec:example1} for the same choice of $\epsilon$ and $[\lb{\alpha},\ub{\alpha}]=[0.5,2.5]$. We first find $M=4$ and $\widehat{\alpha}^\ast=3.0961$ and note that $\ub{\alpha}$ satisfies condition~\eqref{eq:boundOnAlphaUb} since
$\ub{\alpha} = 2.5 \leq \widehat{\alpha}^\ast \,(1+\epsilon)^{-1} =2.8146$.
We next compute $\Rr_M^1$ and $\Y^{[\lb{\alpha},\ub{\alpha}]}_M$ according to~\eqref{eq:sequenceRk} and Lem.~\ref{lem:YalphaConvY1}, respectively, and derive the scaled set $E^\epsilon \,\Y^{[\lb{\alpha},\ub{\alpha}]}_M$ which is shown in Fig.. 
Finally, we identify the smallest $N$ that satisfies~\eqref{eq:NConditionApprox} for $\widehat{\eta}=0.1925$ and obtain $N=3$.  

\begin{figure}[!h] 
\psfrag{xl3}[c][c]{\small$0.8$}
\psfrag{xl2}[c][c]{\small$0$}
\psfrag{xl1}[c][c]{\small$-0.8$}
\psfrag{yl3}[c][c]{\small\,$0.8$}
\psfrag{yl2}[c][c]{\small\,$0$}
\psfrag{yl1}[c][c]{\small$-0.8$}
\psfrag{zl3}[r][r]{\small\,$2.5$}
\psfrag{zl2}[r][r]{\small\,$1.5$}
\psfrag{zl1}[r][r]{\small$0.5$}
\psfrag{x1}[c][c]{$x_1$}
\psfrag{x2}[ct][cb]{$x_2$}
\psfrag{a}[c][c]{$\alpha$}

\includegraphics[trim=1mm 3mm 0mm 0mm, clip=true,width=1.0\linewidth]{figYMExample2.eps}
\caption{
Illustration of .
} 
\label{fig:figYMExample2}
\end{figure}

\begin{figure}[!h] 
\psfrag{xl3}[c][c]{\small$4$}
\psfrag{xl2}[c][c]{\small$0$}
\psfrag{xl1}[c][c]{\small$-4$}
\psfrag{yl3}[c][c]{\small\,$3$}
\psfrag{yl2}[c][c]{\small\,$0$}
\psfrag{yl1}[c][c]{\small$-3$}
\psfrag{zl3}[r][r]{\small\,$2.5$}
\psfrag{zl2}[r][r]{\small\,$1.5$}
\psfrag{zl1}[r][r]{\small$0.5$}
\psfrag{x1}[c][c]{$x_1$}
...
\includegraphics[trim=1mm 3mm 0mm 0mm, clip=true,width=1.0\linewidth]{figZNExample2.eps}
\caption{
Illustration of .
} 
\label{fig:figZNExample2}
\end{figure}
