...
In fact, for state constraints $\X$ of the form
$$
\X = \{ x \in \R^n \,|\, H x \leq d \}
$$
with $H \in \R^{L \times n}$ and $d \in \R^L$,
the sets $\Ss_k^\alpha$ (from~\eqref{eq:sequenceSk}) can be expressed as
$$
...
Thereby, $h_{\W^\alpha} (H A^i)$ can be computed by solving $L$ linear programs. In addition, it is easy to see that
$$
...
\nonumber
\lb{\alpha} \leq z_{n+1} \leq \ub{\alpha},  \,\, \forall  j \in \N_{[0,k]} \bigg\}
\end{align}

This is discussed in more detail in \cite{Schaich2015}.

\subsection{Example 1}

We first consider system~\eqref{eq:system} with 
$A=0.5$
and the constraints $\X=[-2,2]$ and $\W^\ast=[-1,1]$.
