...
\begin{equation}
\label{eq:sequenceTk}
\T_{k+1} := A^{-1} (\T_k \ominus \W^\ast) \cap \X \quad \text{with} \quad  \T_{0}:=\X \cap (1+\epsilon) \Rr_M^1 ,
\end{equation}
...
\section{Computation for polytopic sets}
...
\begin{align}
\nonumber
\Z_k^{[\lb{\alpha},\ub{\alpha}]}= \Big\{ z \in \R^{n+1} \,\Big|\, \begin{pmatrix} H A^j & \sum \limits_{i=0}^{j-1} h_{\W^\ast}(H A^i)  \end{pmatrix} z \leq d, \,\,\,\\
\nonumber
\lb{\alpha} \leq z_{n+1} \leq \ub{\alpha},  \,\, \forall  j \in \N_{[0,k]}, \Big\}
\end{align}
